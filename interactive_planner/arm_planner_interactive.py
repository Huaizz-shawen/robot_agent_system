# arm_planner_interactive.py

"""
Interactive Robotic Arm Planner
Implements closed-loop, single-step planning with feedback
"""
...
from arm_prompt_template_interactive import (
    get_arm_interactive_system_prompt,
    get_arm_interactive_qwen_config,
...
class InteractiveArmPlanner:
    """
    Interactive UR5e Robotic Arm Planner

    This planner operates in closed-loop mode:
    1. Plan ONE step based on current context + history
    2. Execute that step
    3. Wait for feedback/observation
    ...
    def __init__(self, api_key=None, model_name=None):
        """
        Initialize interactive arm planner
        ...
        # Get configuration
        self.config = get_arm_interactive_qwen_config(model_name)
        ...
        self.system_prompt = get_arm_interactive_system_prompt()

        # Conversation state
        self.reset_conversation()

        print(f"✅ Initialized InteractiveArmPlanner with model: {self.config['model']}")
    ...
    def start_new_task(self, request):
        """
        Start a new task from request

        Args:
            request: Natural language request (from humanoid robot or direct)
    ...
    def provide_feedback(self, feedback):
        ...
        Args:
            feedback: Observation or feedback about the executed step
    ...
    def execute_step_and_wait(self, step_plan):
        """
        Execute a step (simulation) and wait for feedback

        Args:
            step_plan: The step plan from plan_next_step()

        Returns:
            str: Prompt for feedback
        """
        ...
        feedback_request = step_plan.get("feedback_request", "Please provide feedback on the executed step.")
...
def main():
    ...
    print("🦾 Interactive Robotic Arm (UR5e) Task Planner")
    print("="*70)
    ...
    try:
        ...
        while True:
            ...
            if current_step_plan is None or planner.is_task_complete:
                ...
                if current_step_plan and "error" not in current_step_plan:
                    if not planner.is_task_complete:
                        # Execute and wait for feedback
                        feedback_request = planner.execute_step_and_wait(current_step_plan)
                        print(f"\n❓ {feedback_request}")
            else:
