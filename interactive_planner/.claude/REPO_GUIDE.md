...
- **Text-based** observations from human feedback
- **Step-by-step** human confirmation
- **Text-only LLM** (qwen-turbo/plus/max)
- Use for: teaching, demos, safety-critical tasks
...
```
humanoid_planner_interactive.py (494 lines)
├─ InteractiveHumanoidPlanner - Main planner for Unitree-G1 humanoid robots
├─ start_new_task(request) - Begin new task
├─ plan_next_step() - Generate single next step
├─ provide_feedback(feedback) - Update context & plan next
└─ execute_step_and_wait(step) - Execute & get feedback

arm_planner_interactive.py (433 lines)
└─ InteractiveArmPlanner - Planner for UR5e robotic arms (similar API)

humanoid_executor.py (487 lines)
├─ HumanoidExecutor - Executes actions on robots
└─ execute_action() - Runs Talk/Tool/Act/Sense actions
```
...
**Interactive Mode:**
```
test/test_interactive_mock.py - Mock tests (no API calls needed)
test/interactive_planner_usage.py - 4 complete usage scenarios
test/humanoid_usage_example.py - Humanoid-specific examples
test/arm_planner_interactive_usage.py - Robotic arm examples
```
