...
from humanoid_prompt_template_vlm import (
...
from humanoid_executor_vision import VisionEnabledExecutor
from qwen_vlm_client import QwenVLMClient


class AutonomousVLMPlanner:
    """
    Autonomous Humanoid Robot Planner with Vision-Language Model

    Key features:
    - VLM planner receives images directly (not text descriptions)
    - Autonomous execution loop with visual feedback
    - Human interaction only for: task requests, clarifications, completions
    - Visual verification of action success/failure
    """
    ...
    def start_new_task(self, human_request: str, run_autonomously: bool = True) -> Dict:
        ...
        if self.verbose:
            print("\n" + "="*70)
            print("🤖 NEW TASK STARTED (VLM-Based Autonomous Mode)")
            print("="*70)
    ...
    def _run_autonomous_loop(self) -> Dict:
        """
        Run fully autonomous execution loop until completion or human input needed
    ...
    def _execute_step(self, step_plan: Dict):
        ...
        if self.verbose:
            ...
            print(f"Feedback: {execution_result.feedback}")
            if execution_result.data.get('observation_image'):
                print(f"New Observation: {execution_result.data['observation_image']}")
            print(f"{'='*70}\n")
    ...
    def _display_step_plan(self, step_plan: Dict):
        ...
        if next_step:
            ...
            print(f"  Expected Visual Outcome: {next_step.get('expected_visual_outcome', 'N/A')}")
