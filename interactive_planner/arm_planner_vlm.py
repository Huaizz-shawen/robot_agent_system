...
from arm_prompt_template_vlm import (
...
class AutonomousArmVLMPlanner:
    """
    Autonomous Robotic Arm Planner with Vision-Language Model
    ...
    def start_new_task(self, request: str, run_autonomously: bool = True,
        ...
        if self.verbose:
            print("\n" + "="*70)
            print("🦾 NEW ARM TASK STARTED (VLM-Based Autonomous Mode)")
            print("="*70)
    ...
    def _run_autonomous_loop(self) -> Dict:
        """
        Run fully autonomous execution loop until completion or humanoid input needed
    ...
    def _execute_step_simulation(self, step_plan: Dict):
        """
        Simulate step execution (updates observation image)
        ...
        if self.verbose:
            print(f"\n✅ Execution simulated")
            print(f"📸 New observation: {self.current_observation_image}\n")
    ...
    def _update_observation_after_action(self, next_step: Dict):
        ...
        elif action == "communicate_with_humanoid":
            # No visual change
            pass
        else:
